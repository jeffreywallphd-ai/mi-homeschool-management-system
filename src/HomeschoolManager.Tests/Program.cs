...
var tests = new List<(string Name, Func<Task> Test)>
{
    ...
    ("Production settings create local-first folders and persist portal choices", () =>
    {
        ...
        AssertEqual(ProductionAvailabilityMode.AlwaysAvailable, settings.AvailabilityMode, "Production settings should prefer Always Available student access by default.");
    ...
    ("Windows release script preserves data outside the app layout and invokes Velopack packaging", () =>
    {
        ...
        var setupProject = File.ReadAllText(Path.Combine(root, "src", "HomeschoolManager.Setup", "HomeschoolManager.Setup.csproj"));
        var setupProgram = File.ReadAllText(Path.Combine(root, "src", "HomeschoolManager.Setup", "Program.cs"));
        var solution = File.ReadAllText(Path.Combine(root, "HomeschoolManager.sln"));
        ...
        var localInstallDoc = File.ReadAllText(Path.Combine(root, "docs", "operations", "local-installation-and-data-location.md"));
        var installerAdr = File.ReadAllText(Path.Combine(root, "docs", "adr", "ADR-0009-family-setup-maintenance-wrapper.md"));
        ...
        AssertTrue(script.Contains("src/HomeschoolManager.Setup/HomeschoolManager.Setup.csproj", StringComparison.Ordinal), "Release script should publish the family setup and maintenance app.");
        AssertTrue(script.Contains("-p:PublishSingleFile=false", StringComparison.Ordinal), "Family setup should publish as a self-contained folder to avoid single-file publish dependencies.");
        AssertTrue(script.Contains("DisableNuGetAudit", StringComparison.Ordinal), "Release script should support local layout smoke tests when NuGet audit services are unavailable.");
        AssertTrue(script.Contains("HomeschoolManager-Family-Setup\\HomeschoolManager-Family-Setup.exe", StringComparison.Ordinal), "Release script should copy the family-facing setup app into packages.");
        AssertTrue(script.Contains("Copy-Item -Path (Join-Path $setupRoot \"*\")", StringComparison.Ordinal), "Release script should expand the setup publish folder wildcard when copying the family setup bundle.");
        AssertTrue(script.Contains("familySetup = \"packages/HomeschoolManager-Family-Setup/HomeschoolManager-Family-Setup.exe\"", StringComparison.Ordinal), "Release manifest should name the family setup wrapper.");
        ...
        AssertTrue(script.Contains("defaultAvailability = \"AlwaysAvailable\"", StringComparison.Ordinal), "Release manifest should document the recommended production availability mode.");
        ...
        AssertTrue(script.Contains("enable-always-available.ps1", StringComparison.Ordinal), "Release layout should include the friendly Always Available setup helper.");
        AssertTrue(script.Contains("disable-always-available.ps1", StringComparison.Ordinal), "Release layout should include the friendly Always Available removal helper.");
        ...
        AssertTrue(File.Exists(Path.Combine(root, "tools", "release", "enable-always-available.ps1")), "Friendly Always Available setup helper should exist.");
        AssertTrue(File.Exists(Path.Combine(root, "tools", "release", "disable-always-available.ps1")), "Friendly Always Available removal helper should exist.");
        ...
        AssertTrue(solution.Contains("HomeschoolManager.Setup", StringComparison.Ordinal), "Solution should include the setup and maintenance app.");
        AssertTrue(setupProject.Contains("<UseWindowsForms>true</UseWindowsForms>", StringComparison.Ordinal), "Setup app should use a normal Windows UI instead of a terminal flow.");
        AssertTrue(setupProject.Contains("<OutputType>WinExe</OutputType>", StringComparison.Ordinal), "Setup app should not open a console window.");
        AssertTrue(setupProgram.Contains("Always Available (recommended)", StringComparison.Ordinal), "Setup app should default parents toward Always Available.");
        AssertTrue(setupProgram.Contains("Open Only", StringComparison.Ordinal), "Setup app should keep Open Only available.");
        AssertTrue(setupProgram.Contains("UpdatePrimaryActionVisibility", StringComparison.Ordinal), "Setup app should keep install and uninstall actions visible in the shared command bar.");
        AssertTrue(setupProgram.Contains("tabs.SelectedIndexChanged", StringComparison.Ordinal), "Setup app should switch the visible primary action when the selected maintenance tab changes.");
        AssertTrue(setupProgram.Contains("tabs.SelectedIndex < 0", StringComparison.Ordinal), "Setup app should initialize the install tab when WinForms has not finalized the selected tab yet.");
        AssertTrue(setupProgram.Contains("UpdatePrimaryActionVisibility();", StringComparison.Ordinal), "Setup app should refresh the primary action during startup.");
        AssertTrue(setupProgram.Contains("tabs.SelectedIndex <= 0", StringComparison.Ordinal), "Install action should remain visible before and during the install/repair tab selection.");
        AssertTrue(setupProgram.Contains("uninstallButton.Visible = !showInstallAction", StringComparison.Ordinal), "Uninstall action should remain visible on the uninstall tab.");
        AssertTrue(setupProgram.Contains("PerformLayout", StringComparison.Ordinal), "Setup app should force the command bar to recalculate after primary action visibility changes.");
        AssertTrue(setupProgram.Contains("Remove Family Records", StringComparison.Ordinal), "Setup app should require exact confirmation before destructive data removal.");
        AssertTrue(setupProgram.Contains("CreateSafetyArchive", StringComparison.Ordinal), "Setup app should create a safety archive before selected family-record removal.");
        AssertTrue(setupProgram.Contains("HomeschoolManagerOriginalUninstallString", StringComparison.Ordinal), "Setup app should preserve the Velopack uninstall command before registering the maintenance prompt.");
        AssertTrue(setupProgram.Contains("HomeschoolManagerMaintenance", StringComparison.Ordinal), "Setup app should not overwrite the original uninstall command after maintenance registration owns Add/Remove.");
        AssertTrue(setupProgram.Contains("HomeschoolManagerDataRetention", StringComparison.Ordinal), "Setup app should mark the uninstall entry as data-retention prompted.");
        AssertTrue(setupProgram.Contains("Registry.CurrentUser", StringComparison.Ordinal), "Setup app should register the per-user maintenance uninstall prompt.");
        AssertTrue(setupProgram.Contains("--uninstall", StringComparison.Ordinal), "Setup app should support Windows Add/Remove invoking the uninstall prompt.");
        ...
        AssertTrue(hostProgram.Contains("HomeschoolManager__ProductionAvailabilityMode", StringComparison.Ordinal), "Portal child processes should receive the parent-facing availability preference.");
        ...
        AssertTrue(installServiceScript.Contains("--availability-mode\", \"AlwaysAvailable\"", StringComparison.Ordinal), "Service install helper should write the Always Available preference.");
        ...
        AssertTrue(setupPage.Contains("Student access", StringComparison.Ordinal), "Setup should show parent-friendly student access availability.");
        AssertTrue(setupPage.Contains("HomeschoolManager-Family-Setup.exe", StringComparison.Ordinal), "Setup should point raw-package installs back to the family setup wrapper.");
        AssertTrue(setupPage.Contains("enable-always-available.ps1", StringComparison.Ordinal), "Setup should direct parents to the friendly Always Available helper when needed.");
        AssertTrue(productionStatusService.Contains("Always Available (recommended)", StringComparison.Ordinal), "Production status service should describe the recommended availability mode in parent-friendly language.");
        AssertTrue(productionStatusService.Contains("Open Only", StringComparison.Ordinal), "Production status service should keep the fallback mode parent-friendly.");
        ...
        AssertTrue(releaseReadme.Contains("HomeschoolManager-Family-Setup.exe", StringComparison.Ordinal), "Release docs should direct families to the setup wrapper.");
        AssertTrue(releaseReadme.Contains("Windows Desktop publish/runtime support packages", StringComparison.Ordinal), "Release docs should name the setup publish dependency.");
        AssertTrue(releaseReadme.Contains("-DisableNuGetAudit", StringComparison.Ordinal), "Release docs should document local audit bypass for smoke tests.");
        AssertTrue(releaseReadme.Contains("Do not give nontechnical families `HomeschoolManager-stable-Setup.exe`", StringComparison.Ordinal), "Release docs should warn against using raw Velopack as the family installer.");
        AssertTrue(releaseReadme.Contains("Remove Family Records", StringComparison.Ordinal), "Release docs should document the uninstall confirmation text.");
        AssertTrue(releaseReadme.Contains("Always Available Student Access", StringComparison.Ordinal), "Release docs should explain the recommended availability setup.");
        AssertTrue(backgroundServiceDoc.Contains("Always Available mode is the recommended production setup", StringComparison.Ordinal), "Operations docs should explain Always Available mode in parent-friendly language.");
        AssertTrue(backgroundServiceDoc.Contains("Open Only mode remains available", StringComparison.Ordinal), "Operations docs should preserve the Open Only fallback.");
        AssertTrue(localInstallDoc.Contains("Windows Add/Remove should route uninstall to the Homeschool Manager maintenance prompt", StringComparison.Ordinal), "Operations docs should describe the maintenance uninstall prompt.");
        AssertTrue(installerAdr.Contains("HomeschoolManager-Family-Setup.exe", StringComparison.Ordinal), "ADR should record the family setup wrapper.");
        AssertTrue(installerAdr.Contains("Raw Velopack `Setup.exe` remains an advanced/package artifact", StringComparison.Ordinal), "ADR should keep raw Velopack out of the parent-facing installer role.");
        return Task.CompletedTask;
    })
};

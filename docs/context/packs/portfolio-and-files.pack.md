# Portfolio and Files Pack

Purpose: Portfolio artifacts, evidence tags, stored files, and artifact exports.

## Canonical Sources

- `docs/domain/portfolio-evidence-rules.md`
- `docs/domain/file-and-artifact-taxonomy.md`
- `docs/product/homesteading-portfolio-use-cases.md`
- `docs/architecture/local-data-and-file-storage.md`

## Must Preserve

- Files are content; artifacts are educational evidence.
- Exports preserve artifact context and metadata.
- Practical work can support academic evidence through course descriptions and tags.

## Common Failure Modes

- Exporting files without context.
- Using file paths as domain identity.
